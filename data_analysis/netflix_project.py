...
# 세션이 저장소에 업로드한 csv 파일을 읽어 변수에 할당
...
for i in netflix.columns :
    ...
    if missingValueRate > 0 :
        print("{} null rete: {}%".format(i,round(missingValueRate, 2)))
...
netflix.to_csv('data_analysis/netflix_preprocessed.csv', index=False)
...
netflix = pd.read_csv('data_analysis/netflix_preprocessed.csv')
...
# title열의 값이 'Sankofa'인 행 전체를 확인하여 country 열과 age_group열의 값이 어떻게 이루어져 있는지 확인
...
plt.suptitle('Target ages proportion of content by country',
